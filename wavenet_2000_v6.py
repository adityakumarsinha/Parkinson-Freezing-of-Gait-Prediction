...
class GaitDataset(torch.utils.data.Dataset):
    ...
    def get_defog_data(self,row):
        wid = row.Id
        files = glob.glob(f'{INPUT_PATH}/train/defog_np_v2/{wid}*.npy')
    ...
    def __getitem__(self, idx):
        ...
        if self.is_train:
            if 0:#row.type == 1 and np.random.uniform() < 0.5:
                ...
                if row.type == 1:
                    wav = np.load(f'{INPUT_PATH}/train/defog_np//{wid}_sig.npy')
                    tgt = np.load(f'{INPUT_PATH}/train/defog_np/{wid}_tgt.npy')
                    mask = pd.read_csv(f'{INPUT_PATH}/train/defog/{wid}.csv').Valid.values
...
def save_model(epoch,model,ckpt_path='./',name='',score=0):
    path = os.path.join(ckpt_path, '{}_{}_{:.3f}.pth'.format(name, epoch, score))
...
hparams = {
    # Optional hparams
    "backbone": 'wavenet_2000', #'', #'tf_efficientnetv2_b2',
    "learning_rate": [5e-4],
    "max_epochs": 71,
    "batch_size": 16,
    "num_workers": 0,
...
defog[defog.fold==fn].shape,defog[defog.fold!=fn].shape
